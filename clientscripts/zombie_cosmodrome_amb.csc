...
samantha_is_angry_setup()
{
    ...
    {
        player[i] thread samantha_is_angry_earthquake_and_rumbles(i);
    }
}

samantha_is_angry_earthquake_and_rumbles(clientnum)
{
    self endon("death");
	...
	clientscripts\apex\_utility::visionset_activate(clientnum, "cosmodrome_nopower");
	// self clientscripts\_zombiemode::zombie_vision_set_apply( level._visionset_map_nopower, 8 );
	self thread do_that_sam_rumble();
	wait(6);
	snd_set_snapshot( "default" );
	clientscripts\apex\_utility::visionset_deactivate(clientnum, "cosmodrome_nopower");
